  ...
  return (
    <a href="#top" className="group inline-flex items-center gap-3" aria-label="CallCare BPO home">
      ...
      <span className={`font-display text-[16px] font-bold tracking-[-.05em] ${light ? "text-white" : "text-[#123d31]"}`}>
        CALLCARE <span className={light ? "text-[#d99a3d]" : "text-[#4a7d6b]"}>BPO</span><span className={`ml-2 align-middle font-sans text-[9px] font-bold tracking-[.12em] ${light ? "text-[#b8d0c3]" : "text-[#88a59a]"}`}>/ OPERATIONS</span>
      </span>
    </a>
  );
}
...
export default function Home() {
  ...
  return (
    <div id="top" className="min-h-screen overflow-hidden bg-[#f6f3ec] text-[#163b31]">
      ...
      <main>
        <section className="hero-noise relative isolate min-h-[720px] overflow-hidden bg-[#073a2d] text-white lg:min-h-[760px]">
          ...
          <div className="absolute inset-y-0 right-0 -z-10 hidden w-[28%] bg-[#d99a3d]/10 lg:block [clip-path:polygon(35%_0,100%_0,100%_100%,0%_100%)]" />
          <div className="relative mx-auto flex min-h-[720px] max-w-[1280px] items-end px-5 pb-16 pt-36 lg:min-h-[760px] lg:px-10 lg:pb-24"><span className="absolute left-5 top-32 hidden size-3 rounded-full bg-[#d99a3d] shadow-[0_0_0_7px_rgba(217,154,61,.12)] lg:block" />
            <div className="max-w-[760px] reveal">
              ...
              <h1 className="font-display max-w-[760px] text-balance text-[clamp(3.25rem,7vw,7.15rem)] font-semibold leading-[.92] tracking-[-.075em]">Put the busywork in <span className="text-[#d99a3d]">capable hands.</span></h1>
        ...
        <section id="about" className="paper-grid relative px-5 py-24 lg:px-10 lg:py-32">
        ...
        <section id="industries" className="bg-[#f6f3ec] px-5 py-24 lg:px-10 lg:py-32"><div className="mx-auto grid max-w-[1280px] gap-14 lg:grid-cols-[1fr_1fr] lg:gap-24"><div><SectionLabel index="03" label="Who we help" /><h2 className="mt-10 max-w-[550px] font-display text-[clamp(2.6rem,5vw,5rem)] font-semibold leading-[.95] tracking-[-.065em]">A good fit for businesses with room to grow.</h2><p className="mt-8 max-w-[470px] text-[17px] leading-8 text-[#57776b]">From startups to established companies, we provide the support your business needs to run smoothly and scale faster.</p><a href="#contact" className="mt-9 inline-flex items-center gap-2 border-b-2 border-[#d99a3d] pb-2 text-sm font-bold text-[#163b31]">Find your support model <ArrowUpRight size={16} /></a></div><div className="grid grid-cols-2 gap-px bg-[#b8cbc0]">{industries.map((industry, i) => <div key={industry} className="group flex min-h-[130px] flex-col justify-between bg-[#f6f3ec] p-5 transition-colors hover:bg-[#e7f0ea] sm:min-h-[160px] sm:p-7"><span className="text-[11px] font-bold tracking-[.12em] text-[#88a59a]">0{i + 1}</span><div className="flex items-end justify-between"><span className="font-display text-lg font-semibold tracking-[-.03em] text-[#315b4d]">{industry}</span><ArrowUpRight size={17} className="text-[#d99a3d] opacity-0 transition-opacity group-hover:opacity-100" /></div></div>)}</div></div></section>

        <section id="process" className="bg-[#0b4f3b] px-5 py-24 text-white lg:px-10 lg:py-32"><div className="mx-auto max-w-[1280px]"><div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end"><div><SectionLabel index="04" label="How we work" light /><h2 className="mt-9 max-w-[700px] font-display text-[clamp(2.7rem,5vw,5.4rem)] font-semibold leading-[.95] tracking-[-.07em]">A simple partnership process, built around your reality.</h2></div><p className="max-w-[290px] text-sm leading-6 text-[#b8d0c3]">We do the listening first, then build the support model around the work that needs to move.</p></div><div className="mt-20 grid gap-0 border-t border-white/20 lg:grid-cols-5">{[['01','Understand','We learn about your business, challenges, and goals.'],['02','Build','We recommend the right support model and plan.'],['03','Match','We find and train the right professionals.'],['04','Integrate','We onboard our team into your tools and processes.'],['05','Improve','We monitor performance, gather feedback, and improve.']].map(([n,title,desc]) => <div key={n} className="border-b border-white/20 py-7 lg:border-b-0 lg:border-r lg:px-6 lg:first:pl-0 lg:last:border-r-0"><span className="font-display text-4xl text-[#d99a3d]">{n}</span><h3 className="mt-9 font-display text-lg font-semibold">{title}</h3><p className="mt-3 max-w-[180px] text-sm leading-6 text-[#b8d0c3]">{desc}</p></div>)}</div></div></section>

        <section className="paper-grid px-5 py-24 lg:px-10 lg:py-32"><div className="mx-auto max-w-[1280px]"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><SectionLabel index="05" label="Flexible engagement" /><h2 className="mt-9 max-w-[650px] font-display text-[clamp(2.6rem,5vw,5rem)] font-semibold leading-[.96] tracking-[-.07em]">Start where you are. Scale when you’re ready.</h2></div><p className="max-w-[300px] text-sm leading-6 text-[#57776b]">Indicative packages from the service brochure. We’ll shape the right scope together.</p></div><div className="mt-14 grid gap-4 lg:grid-cols-3">{[['Starter','From $800/month','Dedicated remote professional','Part-time support','Administrative or customer support'],['Growth','From $1,500/month','Full-time specialist','Business operations support','Reporting and administrative assistance'],['Scale','From $3,000/month','Team of specialists','Customer, admin, and growth support','Creative and data support']].map(([name,price,...items], i) => <div key={name} className={`relative flex min-h-[340px] flex-col justify-between border p-7 ${i === 1 ? 'border-[#0b4f3b] bg-[#0b4f3b] text-[#f6f3ec]' : 'border-[#b8cbc0] bg-[#f6f3ec]'}`}><div><div className="mb-12 flex items-center justify-between"><span className="eyebrow opacity-70">0{i + 1} / package</span>{i === 1 && <span className="rounded-full bg-[#d99a3d] px-3 py-1 text-[10px] font-bold uppercase tracking-[.1em] text-[#163b31]">Popular path</span>}</div><h3 className="font-display text-3xl font-semibold tracking-[-.05em]">{name}</h3><p className={`mt-2 text-sm font-bold ${i === 1 ? 'text-[#d99a3d]' : 'text-[#0b4f3b]'}`}>{price}</p></div><div className={`space-y-2 border-t pt-5 text-sm ${i === 1 ? 'border-white/20 text-[#c7ddd0]' : 'border-[#c7d5ca] text-[#57776b]'}`}>{items.map(item => <div key={item} className="flex gap-2"><Check size={15} className="mt-0.5 shrink-0 text-[#d99a3d]" />{item}</div>)}</div></div>)}</div></div></section>
